...
import useScan from '../hooks/useScan.js'; // Connected your custom hook layer
...
export default function ResultCard({ result }) {
  ...
  const {reportToCommunityHook, reportError, reportLoading} = useReport()
console.log(result)
  if (!result) return null;

  // Added fallback support for both naming patterns ('scamMessage' and 'inputContent')
  const {scamId, verdict, confidence, reasonHindi, reasonEnglish, redFlags = [], inputContent, scamMessage, inputType } = result;
  const contentToReport = inputContent || scamMessage || 'User scanned query content';

  const handleReport = async () => {    
      reportToCommunityHook(scamId)
  ...
  return (
    <div className="w-full max-w-2xl bg-white border border-gray-200 rounded-xl shadow-sm p-6 md:p-8 space-y-6">
      ...
      {contentToReport && contentToReport !== 'User scanned query content' && (
