...
const useScan = () => {
    ...
    const scamDetectOfTextHook = useCallback(async (message, language = 'English') => {
        try {
            return await dispatch(scamDetectOfTextSlice({ message, language }));
        } catch(err) {
            throw err;
        }
    }, [dispatch]);

    const scamDetectOfUrlHook = useCallback(async (url, language = 'English') => {
        try {
            return await dispatch(scamDetectOfUrlSlice({ url, language }));
        } catch(err) {
            throw err;
        }
    }, [dispatch]);

    const scamDetectOfImageHook = useCallback(async (image, language = 'English') => {
        try {
            return await dispatch(scamDetectOfImageSlice({ image, language }));
        } catch(err) {
            throw err;
        }
    }, [dispatch]);
